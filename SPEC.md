...
- Embedding the app in the owner's own website (e.g. a WordPress page) via an `<iframe>` is optional future work: that website is not live yet and no custom domain is configured. The app stands on its own at `https://avatar-sergei.fly.dev`. Notes for the embed:
  ...
  - The app never sets `X-Frame-Options`, so it can always be framed by someone. `FRAME_ANCESTORS` (space-separated origins, set in `scripts/fly.toml` `[env]`) narrows that: when it is set, every response carries `Content-Security-Policy: frame-ancestors 'self' <origins>`, so only the owner's own site can embed the app. Unset - the default, and what local development uses - leaves framing open.
