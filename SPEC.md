...
- There is an OpenRouter API key in the .env file. The model is read from `MODEL` in `.env`. `openai/gpt-5.4-nano` is the cheap default for development and testing (also the code default in `config.py`); the reference production deployment uses `openai/gpt-5.6-luna`. Each owner sets their own `MODEL` (an OpenRouter `openai/...` identifier; see Q&A #2).
...
- The platform is deployed to fly.io as the single container the Dockerfile and `scripts/` build and run; the mechanics (app, region, secrets, custom domain, smoke tests) are documented in `DEPLOY.md`, driven by `scripts/fly.toml` and `scripts/deploy.sh`. This deployment runs as app `avatar-sergei` in region `lhr` (London, the closest Fly region to the Supabase project in eu-west-1 / Ireland), on a `shared-cpu-1x` machine with 512 MB RAM, always on. Notes for embedding it in an existing site (e.g. a WordPress page) via an `<iframe>` - **optional and not part of this build**, since the owner's website is not deployed yet; the app stands on its own at `https://avatar-sergei.fly.dev`:
...
2. next_level.ipynb is a Jupyter Notebook with code to (a) make use of the json FAQ file for quick answers (b) support a shortcut way to ask for a question just by typing "Q2" that doesn't require an LLM call (c) streaming back, including the tool usage. I've not included the CSS but it showed tool use in a small font.
   **Scope note:** this notebook is an *Agentic RAG* experiment and also wires up a Qdrant vector database over MCP (built by an `ingest.ipynb` that is not part of this repo). **That part is out of scope** - per Q&A #3 this project has NO vector DB and NO MCP server, and the knowledge comes from `knowledge/` composed into the system prompt. Take only the `faq_tool`, the `Qn` instant-answer shortcut, and the streaming/tool-display patterns from it; ignore everything Qdrant, MCP and `ingest`-related.
...
- **Owner of this build:** this site belongs to **Sergei Maslennikov** (LLM engineer / AI practitioner, Tallinn, Estonia), not to the original author of the template. Everything owner-specific has already been prepared - do not re-derive it and do not carry over any copy, links or identity from the template author:
  - The PNGs in `design-system/assets/` are **already regenerated** from this owner's `knowledge/pic.jpg`. Use them as they are; do NOT run the recipe in `design-system/docs/avatar-generation.md` again.
  - `knowledge/knowledge.md`, `style.md` and `faq.jsonl` are this owner's. All UI copy (brand subtitle, intro screen, example prompts, page title, meta description) must be derived from them, and must never mention the template author.
  - **Footer social links:** LinkedIn `https://www.linkedin.com/in/sergei-maslennikov-ai`, GitHub `https://github.com/Neuromediator`, Hugging Face `https://huggingface.co/Neuromediator`. There is **no YouTube link** - do not add one.
  - The owner's name comes from the `OWNER_NAME` env var (`Sergei Maslennikov`) and is shown in the UI, including the human bubble (e.g. "Sergei Maslennikov - live"); it must always be read from that config and never hardcoded (per Q&A #4 and #11).
...
1. **Supabase credentials.** Done. The Supabase project (region eu-west-1, Ireland), the `messages` table and all `.env` values are in place, and `backend/tests/test_supabase_connection.py` passes.

2. **Model.** The model name is read from the `MODEL` env var (OpenRouter `openai/...` prefix). `openai/gpt-5.4-nano` is the cheap default for development and testing (and the code default in `config.py`); the reference production deployment uses `openai/gpt-5.6-luna`. Each owner sets their own.
...
4. **Human-in-the-loop semantics.** When the human posts from admin, the Avatar does NOT react to it. The human's message is inserted into the thread; the full conversation (including it) is provided to the Avatar the next time the visitor submits something. To the visitor, the human's message renders as a separate bubble using the profile pic, distinguished by image + yellow ring + tint + glow (per the design system).
   ...
   **Owner name (updated).** The owner's name comes from the `OWNER_NAME` env var (see #11) and IS shown in the UI, including on the human's bubble (e.g. "Sergei Maslennikov - live") to avoid an awkward anonymous bubble. The name must always be read from `OWNER_NAME` config and NEVER hardcoded.
...
7. **Avatar's robotic icon.** Done - both images ship in `design-system/assets/`, already generated from this owner's `knowledge/pic.jpg`: `avatar-human.png` is the human icon, `avatar-robot.png` / `avatar-robot-round.png` are the Avatar icons. Copy them into the frontend as they are; do not regenerate them.
